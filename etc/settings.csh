...
# appropriate XXX_SYSTEM environment variable for a given package in your prefs.csh
# file in order to disable the activation of the ThirdParty version of the same package.

# These are the default locations for pre-installed version of the ThirdParty
# packages. Those values are based on Ubuntu 20.04
# Use the file prefs.csh to pre-initialize to different default values, accordingly to
# your Linux/Unix flavour
if ( ! $?WM_SYSTEM_DIR )         setenv WM_SYSTEM_DIR         /usr
if ( ! $?WM_SYSTEM_BIN_DIR )     setenv WM_SYSTEM_BIN_DIR     /usr/bin
if ( ! $?WM_SYSTEM_LIB_DIR )     setenv WM_SYSTEM_LIB_DIR     /lib
if ( ! $?WM_SYSTEM_INCLUDE_DIR ) setenv WM_SYSTEM_INCLUDE_DIR /usr/include
...
if ( $?METIS_SYSTEM) then
    ...
    # Some values might already be pre-initialized in prefs.csh
