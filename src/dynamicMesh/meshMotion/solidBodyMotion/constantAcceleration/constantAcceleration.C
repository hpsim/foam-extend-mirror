...
Foam::septernion
Foam::solidBodyMotionFunctions::constantAcceleration::calcTransformation
(
    const scalar t
) const
{
    const vector translation =
        (
            transVelocity_
          + transAcceleration_*(t - startMotionTime_)
        )*(t - startMotionTime_);

    const vector rotation =
        (
            rotVelocity_
          + rotAcceleration_*(t - startMotionTime_)
        )*(t - startMotionTime_);
...
)
{
    ...
    SBMFCoeffs_.lookup("translationalAcceleration") >> transAcceleration_;
    SBMFCoeffs_.lookup("rotationalAcceleration") >> rotAcceleration_;
    ...
    rotAcceleration_ *= inDegrees_ ? mathematicalConstant::pi/180.0 : 1;
