...
#include "overlapGgiEnthalpyJumpFvPatchScalarField.H"
...
:
    jumpOverlapGgiFvPatchField<scalar>(p, iF),
    rotating_(false),
    jump_(this->size(), pTraits<scalar>::zero)
{
    ...
    if (iF.name() == db().mangleFileName("h"))
    {
        ...
        const overlapGgiEnthalpyJumpFvPatchScalarField& Tpatch =
            dynamic_cast<const overlapGgiEnthalpyJumpFvPatchScalarField&>(T);
...
overlapGgiEnthalpyJumpFvPatchScalarField
(
    const overlapGgiEnthalpyJumpFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
    const fvPatchFieldMapper& mapper
)
:
    jumpOverlapGgiFvPatchField<scalar>(ptf, p, iF, mapper),
...
:
    jumpOverlapGgiFvPatchField<scalar>(ptf, iF),
    rotating_(ptf.rotating_),
    jump_(ptf.jump_)
{
}
...
void overlapGgiEnthalpyJumpFvPatchScalarField::autoMap
(
    const fvPatchFieldMapper& m
)
{
    jumpOverlapGgiFvPatchField<scalar>::autoMap(m);
    jump_.autoMap(m);
}


void overlapGgiEnthalpyJumpFvPatchScalarField::rmap
(
    const fvPatchField<scalar>& ptf,
    const labelList& addr
)
{
    jumpOverlapGgiFvPatchField<scalar>::rmap(ptf, addr);

    // rmap jump
    const overlapGgiEnthalpyJumpFvPatchScalarField& ejPtf =
        refCast<const overlapGgiEnthalpyJumpFvPatchScalarField>(ptf);

    jump_.rmap(ejPtf.jump_, addr);
}


void Foam::overlapGgiEnthalpyJumpFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
...
void overlapGgiEnthalpyJumpFvPatchScalarField::write(Ostream& os) const
{
